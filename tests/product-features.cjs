...
(async () => {
  ...
  const before = (await query('select count(*)::integer as n from app_notifications'))[0].n;
  await query('select enqueue_product_reminders()');
  assert.equal((await query('select count(*)::integer as n from app_notifications'))[0].n, before);
